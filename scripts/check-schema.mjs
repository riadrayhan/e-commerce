...
// SAFE: Only create tables if they don't exist. Never drops data.
console.log('Ensuring tables exist (safe — no data will be deleted)...');
await sql`
...
if (productCount === 0) {
  ...
  for (const p of demoProducts) {
    await sql`INSERT INTO products (id, name, price, description, images, image_url, stock, created_at) VALUES (${p.id}, ${p.name}, ${p.price}, ${p.description}, ${JSON.stringify(p.images)}, ${p.images[0]}, ${p.stock}, ${now}) ON CONFLICT (id) DO NOTHING`;
  }
  console.log('Seeded 6 demo products.');
} else {
  console.log(`Products table already has ${productCount} products. No seeding needed.`);
}
...
console.log('Done! All data is safe.');

...
// Retry wrapper to handle Neon cold starts (free tier wakes up slowly)
...
// Track schema initialization — use a promise to avoid races
const g = globalThis as any;
if (!g.__dbInitPromise) {
  g.__dbInitPromise = null;
  g.__dbInitialized = false;
}
...
async function ensureSchema() {
  ...
  g.__dbInitPromise = (async () => {
    try {
      // Verify connection is alive with a simple query first
      await sql`SELECT 1`;
      ...
      if (parseInt(countResult[0].cnt) === 0) {
        ...
        for (const p of demos) {
          await sql`INSERT INTO products (id, name, price, description, images, image_url, stock, created_at) VALUES (${p.id}, ${p.name}, ${p.price}, ${p.desc}, ${JSON.stringify([p.img])}, ${p.img}, ${p.stock}, ${now})`;
        }
      }
...
// Helper to parse JSON fields on a row
...
export async function getProducts() {
  return withRetry(async () => {
    await ensureSchema();
    const rows = await sql`SELECT id, name, price, description, image_url, stock, created_at FROM products WHERE stock > 0 ORDER BY created_at DESC`;
...
export async function createProduct(
  ...
  console.log(`[DB] Product created: id=${id}, name=${name}`);
  return {
